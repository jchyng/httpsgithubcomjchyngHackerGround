...
public class JobPublicCompanyService {
    private final JobPublicCompanyRepository repository;
    private List<JobPublicCompany> jobCompanies = new ArrayList<>();
    ...
    public void crawlingTask() {
        ...
        //DB에 새로운 데이터 저장
        Iterable<JobPublicCompany> jobPublicCompanyIterable = jobCompanies;
        repository.saveAll(jobPublicCompanyIterable);
    ...
    public void saveJob(Document document) {
        ...
        for (Element element : elements) {
            JobPublicCompany jobPublicCompany = new JobPublicCompany(element);
            jobCompanies.add(jobPublicCompany);
        }
    }
}
